...
TASK_NAME = "tech_bottleneck_false_negative_rescue_primary_source_backfill_v1"
...
def _read_queue() -> pd.DataFrame:
    ...
    queue = queue.copy()
    queue["manual_review_entry_class"] = "false_negative_rescue_queue"
    queue["quality_gate_decision"] = "false_negative_rescue_review"
    queue["bottleneck_thesis_support"] = "moderate"
...
def _load_cached(output: Path) -> dict[str, Any] | None:
    summary_path = output / "false_negative_rescue_primary_source_backfill_summary.json"
    required = [
        summary_path,
        output / "false_negative_rescue_backfill_results.csv",
        output / "false_negative_rescue_evidence_matrix.csv",
        output / "false_negative_rescue_gap_matrix.csv",
        output / "false_negative_rescue_manual_approval_candidates.csv",
        output / "false_negative_rescue_remain_pending.csv",
        output / "false_negative_rescue_adjacent_or_reject.csv",
        output / "false_negative_rescue_primary_source_backfill_guardrails.json",
        output / "tech_bottleneck_false_negative_rescue_primary_source_backfill_v1_report.md",
    ]
    if not all(path.exists() for path in required):
        return None
    summary = json.loads(summary_path.read_text(encoding="utf-8"))
    if summary.get("source_false_negative_rescue_count") == EXPECTED_QUEUE_COUNT and summary.get("processed_count") == EXPECTED_QUEUE_COUNT:
        return summary
    return None
...
def _collect_primary_sources(
    ...
    *,
    ...
    manifest_path = output / "false_negative_rescue_primary_source_collection_manifest.csv"
    ...
    search.to_csv(output / "false_negative_rescue_cninfo_search_audit.csv", index=False)
    downloads.to_csv(output / "false_negative_rescue_cninfo_download_audit.csv", index=False)
    return manifest
...
def _parse_text_first(manifest: pd.DataFrame, output: Path, *, max_pages_per_source: int = 80, max_chunks_per_source: int = 10) -> pd.DataFrame:
    chunks_path = output / "false_negative_rescue_text_first_evidence_chunks.csv"
    parse_manifest_path = output / "false_negative_rescue_text_first_parse_manifest.csv"
    ...
    pd.DataFrame(quality_rows).to_csv(output / "false_negative_rescue_text_first_quality_audit.csv", index=False)
    pd.DataFrame(fallback_rows).to_csv(output / "false_negative_rescue_docling_fallback_audit.csv", index=False)
    return pd.DataFrame(chunk_rows) if chunk_rows else _empty_chunks()


def _map_decision(row: dict[str, Any], source_row: pd.Series) -> dict[str, Any]:
    row = dict(row)
    decision = row["recommended_backfill_decision"]
    if decision == "upgrade_to_confirmed_core_proposal":
        row["recommended_backfill_decision"] = "upgrade_to_rescue_manual_approval_candidate"
        row["recommended_manual_review_entry_class"] = "rescue_manual_approval_candidate"
    elif decision == "remain_likely_core_pending_evidence":
        row["recommended_backfill_decision"] = "remain_false_negative_pending"
        row["recommended_manual_review_entry_class"] = "false_negative_pending_evidence"
    ...
    row["original_excluded_reason"] = source_row.get("original_excluded_reason", "")
    row["rescue_reason"] = source_row.get("rescue_reason", "")
...
def _assess_results(queue: pd.DataFrame, evidence_matrix: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in queue.iterrows():
        ...
        rows.append(_map_decision(assessed, row))
    columns = [
        "stock_code",
        "stock_name",
...
def _split_outputs(results: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    upgrades = results[results["recommended_backfill_decision"].eq("upgrade_to_rescue_manual_approval_candidate")]
    pending = results[results["recommended_backfill_decision"].eq("remain_false_negative_pending")]
...
def _summary(queue: pd.DataFrame, results: pd.DataFrame, manifest: pd.DataFrame, evidence: pd.DataFrame, strategy_clean: bool) -> dict[str, Any]:
    ...
    if not strategy_clean or used_for_signal or used_for_admission or price_signal or auto_quality:
        ...
        acceptance = "false_negative_rescue_primary_source_backfill_ready"
    return {
        "task_name": TASK_NAME,
        "source_false_negative_rescue_count": int(len(queue)),
        ...
        "possible_false_negative_processed": False,
        "remain_excluded_processed": False,
        "reject_concept_processed": False,
...
def _guardrails(summary: dict[str, Any]) -> dict[str, Any]:
    return {
        "task_name": TASK_NAME,
        "research_only": True,
        "source_false_negative_rescue_count": summary["source_false_negative_rescue_count"],
        "only_false_negative_rescue_processed": summary["source_false_negative_rescue_count"] == EXPECTED_QUEUE_COUNT
        and summary["processed_count"] == EXPECTED_QUEUE_COUNT,
        "possible_false_negative_processed": summary["possible_false_negative_processed"],
        "remain_excluded_processed": summary["remain_excluded_processed"],
        "reject_concept_processed": summary["reject_concept_processed"],
...
def _report(summary: dict[str, Any]) -> str:
    return "\n".join(
        [
            "# Tech Bottleneck False Negative Rescue Primary Source Backfill v1",
            "",
            "## 1. Scope",
            "This research-only task processes only the 39 likely false-negative rescue candidates. It does not process possible false negatives, remain-excluded names, reject names, or the 172-stock quality pool.",
            "",
            "## 2. Source Collection And Parse",
            f"Source rescue queue: {summary['source_false_negative_rescue_count']}; collection manifest rows: {summary['collection_manifest_count']}; evidence matrix rows: {summary['evidence_matrix_count']}.",
            "",
            "## 3. Backfill Results",
            f"Primary-source supported: {summary['primary_source_supported_count']}; rescue manual approval candidates: {summary['upgrade_count']}; remain pending: {summary['remain_pending_count']}; adjacent watchlist: {summary['adjacent_count']}; downgrade/reject: {summary['downgrade_or_reject_count']}.",
            "",
            "## 4. Guardrails",
            ...
            "1. tech_bottleneck_false_negative_rescue_core_equivalence_gate_v1",
            "2. tech_bottleneck_quality_pool_layer_v2",
            "3. tech_bottleneck_doubler_data_gap_watch_triage_v1",
        ]
    )
...
def run(output_dir: str | Path = OUTPUT_DIR) -> dict[str, Any]:
    ...
    results.to_csv(output / "false_negative_rescue_backfill_results.csv", index=False)
    evidence.to_csv(output / "false_negative_rescue_evidence_matrix.csv", index=False)
    gaps.to_csv(output / "false_negative_rescue_gap_matrix.csv", index=False)
    upgrades.to_csv(output / "false_negative_rescue_manual_approval_candidates.csv", index=False)
    pending.to_csv(output / "false_negative_rescue_remain_pending.csv", index=False)
    adjacent_or_reject.to_csv(output / "false_negative_rescue_adjacent_or_reject.csv", index=False)
    _write_json(output / "false_negative_rescue_primary_source_backfill_summary.json", summary)
    _write_json(output / "false_negative_rescue_primary_source_backfill_guardrails.json", guardrails)
    (output / "tech_bottleneck_false_negative_rescue_primary_source_backfill_v1_report.md").write_text(
        _report(summary),
        encoding="utf-8",
    )
    return summary
