...
CONCEPT_DETAIL_WARNING = "concept sector detail row is unavailable"
    ...
    *,
    ...
    if not detail_row:
        return _empty_detail_payload(
            ...
            sector_id,
            sector_type,
            source="market.concept_daily_bar" if sector_type == "concept" else "market.industry_daily_bar",
            warnings=[
                CONCEPT_DETAIL_WARNING
                if sector_type == "concept"
                else "sector detail row is unavailable"
            ],
        )
    ...
    return {
        ...
        "warnings": warnings,
        "sector_id": str(
            detail_row.get("sector_id")
            or detail_row.get("industry_code")
            or detail_row.get("concept_code")
            or sector_id
        ),
        "sector_name": str(
            detail_row.get("sector_name")
            or detail_row.get("industry_name")
            or detail_row.get("concept_name")
            or sector_id
        ),
    ...
    if sector_type == "concept":
        sql = """
            ...
            WHERE bars.trade_date = %s
              ...
              AND bars.concept_code = %s
        """
        with connect(service) as conn:
            rows = fetch_all(conn, sql, [trade_date, sector_id])
