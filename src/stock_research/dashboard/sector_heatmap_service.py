...
CONCEPT_SOURCE_WARNING = "concept sector heatmap rows are unavailable"
...
def build_sector_heatmap_payload(
    ...
    *,
    ...
    if not rows:
        warnings.append(
            CONCEPT_SOURCE_WARNING
            if sector_type == "concept"
            else "industry sector heatmap rows are unavailable"
        )
    ...
    return {
        ...
        "source": _combine_sources(
            rows,
            default="market.concept_daily_bar" if sector_type == "concept" else "market.industry_daily_bar",
        ),
    ...
    *,
    ...
    if sector_type == "concept":
        sql = """
            ...
            ORDER BY bars.amount DESC NULLS LAST, bars.concept_code
        """
        with connect(service) as conn:
            return [dict(row) for row in fetch_all(conn, sql, [trade_date])]
