...
def validate_official_strategy_result(
    ...
    *,
    ...
    from stock_research.strategy_contracts import StrategyContract

    parameter_contract = StrategyContract(
        contract_id=publication_contract.contract_id,
        strategy_id=publication_contract.strategy_id,
        profile=publication_contract.profile,
        engine=publication_contract.engine_version,
        variant=publication_contract.variant,
        top_n=int(publication_contract.normalized_run_config["top_n"]),
        transaction_cost_bps=float(
            publication_contract.normalized_run_config["transaction_cost_bps"]
        ),
        adjust_type=str(publication_contract.normalized_run_config["adjust_type"]),
        frequency=str(publication_contract.normalized_run_config["rebalance_frequency"]),
        protection_name=(
            str(publication_contract.normalized_run_config["protection_name"])
            if publication_contract.normalized_run_config.get("protection_name")
            else None
        ),
    )
...
def _with_latest_eod_strategy_metrics(strategy: dict[str, Any]) -> dict[str, Any]:
    ...
    if performance_stale:
        ...
        metrics["latest_period_label"] = f"收益估值截止 {performance_as_of_date}"
