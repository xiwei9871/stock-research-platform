...
_PLATFORM_SUMMARY_KEYS = (
    "latest_market_date",
    "latest_market_monitor_date",
    "latest_score_date",
    "latest_factor_date",
    "market_asset_count",
    "score_asset_count",
...
def platform_summary_read_model(payload: dict[str, Any]) -> dict[str, Any]:
    model = {key: payload.get(key) for key in _PLATFORM_SUMMARY_KEYS if key in payload}
    model["topn_preview"] = [
        {key: row.get(key) for key in _SCORE_ROW_KEYS if key in row}
        for row in payload.get("topn_preview") or []
        if isinstance(row, dict)
    ]
    return model


def build_platform_summary_read_model(
    score_version: str = "manual_v1",
    top_n: int = 5,
    service: str = SETTINGS.research_service,
) -> dict[str, Any]:
    materialized = _load_materialized_platform_summary(score_version=score_version, top_n=top_n, service=service)
    if materialized is not None:
        return {**platform_summary_read_model(materialized), "source": "materialized_view"}
    return {**_load_base_table_platform_summary(score_version=score_version, top_n=top_n, service=service), "source": "base_table_fallback"}


def _load_materialized_platform_summary(
    *,
    score_version: str,
    top_n: int,
    service: str,
        ...
        with connect(service) as conn:
            rows = fetch_all(
                ...
                SELECT latest_market_date,
                       ...
                       score_versions,
                       topn_preview
                FROM ops.dashboard_platform_summary_daily
                WHERE score_version = %s
                ORDER BY latest_market_date DESC NULLS LAST
                LIMIT 1
                """,
                [score_version],
            )
    ...
    if not rows:
        return None
    row = dict(rows[0])
    row["topn_preview"] = list(row.get("topn_preview") or [])[:top_n]
    return row


def _load_base_table_platform_summary(
    *,
    score_version: str,
    top_n: int,
    service: str,
    ...
    return {
        ...
        "latest_market_monitor_date": str(market_monitor.get("latest_market_monitor_date") or ""),
