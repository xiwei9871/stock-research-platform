...
def load_shadow_follow_up_queue_summary(
    start_date: str,
    end_date: str,
    limit: int = 20,
    service: str = SETTINGS.research_service,
) -> list[dict[str, Any]]:
    """Return read-only P17 shadow follow-up queue dashboard rows."""
    sql = """
    SELECT
        follow_up_item_id,
        run_id,
    ...
    FROM ops.operator_shadow_follow_up_item
    WHERE follow_up_date BETWEEN %s AND %s
    ORDER BY follow_up_date DESC, priority_bucket, follow_up_status, sample_count DESC, group_key
    LIMIT %s
    """
    try:
...
def _follow_up_row(row: dict[str, Any]) -> dict[str, Any]:
    return {
        "follow_up_item_id": str(row["follow_up_item_id"]),
