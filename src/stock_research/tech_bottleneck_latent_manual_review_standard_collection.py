...
TASK_NAME = "tech_bottleneck_latent_manual_review_standard_collection_v1"
INPUT_QUEUE = (
    PROJECT_ROOT
...
def _load_cached(output: Path) -> dict[str, Any] | None:
    summary_path = output / "latent_manual_review_standard_collection_summary.json"
    required = [
        summary_path,
        output / "latent_manual_review_standard_collection_sources.csv",
        output / "latent_manual_review_standard_collection_download_manifest.csv",
        output / "latent_manual_review_standard_collection_collection_gaps.csv",
        output / "latent_manual_review_standard_collection_guardrails.json",
        output / "tech_bottleneck_latent_manual_review_standard_collection_v1_report.md",
    ]
    if not all(path.exists() for path in required):
        return None
    summary = json.loads(summary_path.read_text(encoding="utf-8"))
    if (
        summary.get("source_standard_collection_queue_count") == EXPECTED_COUNT
        ...
        rows.append(
            {
                ...
                "gap_reason": "No selected CNINFO/SSE/SZSE primary-source PDF was downloaded for this standard manual-review candidate.",
                "recommended_next_action": "retry source collection or use an alternate official disclosure source before standard backfill",
                "research_only": True,
...
def _truthy_count(frame: pd.DataFrame, column: str) -> int:
    if frame.empty or column not in frame.columns:
        return 0
    return int(frame[column].map(lambda value: str(value).strip().lower() in {"true", "1", "yes"}).sum())
...
def _summary(queue: pd.DataFrame, downloads: pd.DataFrame, sources: pd.DataFrame, gaps: pd.DataFrame, strategy_clean: bool) -> dict[str, Any]:
    ...
    used_for_signal = _truthy_count(sources, "used_for_signal")
    used_for_admission = _truthy_count(sources, "used_for_admission")
    blocking = len(queue) != EXPECTED_COUNT or used_for_signal or used_for_admission or not strategy_clean
    if blocking:
        acceptance = "blocked_due_to_guardrail_violation"
    elif len(gaps):
        acceptance = "conditionally_ready_with_collection_gaps"
    else:
        acceptance = "latent_manual_review_standard_collection_ready"
    return {
        "task_name": TASK_NAME,
        "research_only": True,
        "source_standard_collection_queue_count": int(len(queue)),
...
def _guardrails(summary: dict[str, Any]) -> dict[str, Any]:
    return {
        "task_name": TASK_NAME,
        "research_only": True,
        "source_standard_collection_queue_count": summary["source_standard_collection_queue_count"],
...
def _report(summary: dict[str, Any]) -> str:
    return "\n".join(
        [
            "# Tech Bottleneck Latent Manual Review Standard Collection v1",
            "",
            "## 1. Scope",
            "This task collects primary-source disclosures only for the 52-stock latent manual-review standard collection queue. It does not process the completed high-priority batch, human-confirm-first names, quality pool v5, quality pool v6 proposal, or v6 manual approval hold list.",
            "",
...
def run(
    *,
    ...
    sources.to_csv(output / "latent_manual_review_standard_collection_sources.csv", index=False)
    downloads.to_csv(output / "latent_manual_review_standard_collection_download_manifest.csv", index=False)
    gaps.to_csv(output / "latent_manual_review_standard_collection_collection_gaps.csv", index=False)
    _write_json(output / "latent_manual_review_standard_collection_summary.json", summary)
    _write_json(output / "latent_manual_review_standard_collection_guardrails.json", guardrails)
    (output / "tech_bottleneck_latent_manual_review_standard_collection_v1_report.md").write_text(
        _report(summary),
        encoding="utf-8",
    )
    return summary
