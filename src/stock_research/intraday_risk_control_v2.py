...
V2_RISK_PRESETS: dict[str, dict[str, Any]] = {
    "v2_1_strict": {},
    "v2_2_midband": {
...
def resolve_intraday_risk_control_v2_preset(risk_preset: str | None) -> dict[str, Any]:
    preset_name = risk_preset or "v2_1_strict"
    if preset_name not in V2_RISK_PRESETS:
        allowed = ", ".join(sorted(V2_RISK_PRESETS))
        raise ValueError(f"unknown intraday risk control v2 preset: {preset_name}; allowed: {allowed}")
    return dict(V2_RISK_PRESETS[preset_name])
...
def build_intraday_risk_signals_v2(
    ...
    *,
    ...
    reversal_afternoon_mode: str = "zscore",
) -> pd.DataFrame:
    columns = [
        "trade_date",
        "asset_id",
    ...
    tail_threshold = (
        zscore_threshold
        if tail_confirmation_zscore_threshold is None
        else float(tail_confirmation_zscore_threshold)
    )
    reversal_threshold = (
        zscore_threshold
        if reversal_zscore_threshold is None
        else float(reversal_zscore_threshold)
    )
    ...
    afternoon_reversal = afternoon_z.le(-reversal_threshold)
    if reversal_afternoon_mode == "negative_or_zscore":
        afternoon_reversal = afternoon.lt(0.0) | afternoon_reversal
    elif reversal_afternoon_mode != "zscore":
        raise ValueError("reversal_afternoon_mode must be 'zscore' or 'negative_or_zscore'")
    wide["morning_to_afternoon_reversal"] = (
        morning.gt(0.0)
        & afternoon.lt(0.0)
        & morning_z.ge(reversal_threshold)
        & afternoon_reversal
    )
    wide["tail_confirmation_failure"] = (
        last_30m.lt(0.0)
        & close_to_vwap.lt(0.0)
        & tail_z.le(-tail_threshold)
        & vwap_z.le(-tail_threshold)
    )
...
def build_midtrend_risk_states(
    ...
    *,
    ...
    high_escalation_mode: str = "severe_only",
) -> pd.DataFrame:
    columns = [
        "trade_date",
        "asset_id",
    ...
    if high_escalation_mode == "severe_or_current_repeated":
        high_mask = high_mask | (
            frame["midtrend_risk_trigger"]
            & (
                frame["midtrend_risk_trigger_count_5d"].ge(high_5d_count)
                | frame["midtrend_risk_trigger_count_10d"].ge(high_10d_count)
            )
        )
    elif high_escalation_mode != "severe_only":
        raise ValueError(
            "high_escalation_mode must be 'severe_only' or 'severe_or_current_repeated'"
        )
...
def run_intraday_risk_control_v2_from_frames(
    ...
    *,
    ...
    signals = build_intraday_risk_signals_v2(
        ...
        reversal_afternoon_mode=reversal_afternoon_mode,
    )
    states = build_midtrend_risk_states(
        signals,
        high_escalation_mode=high_escalation_mode,
    )
...
def run_intraday_risk_control_v2_backtest(
    ...
    *,
    ...
    preset_kwargs = resolve_intraday_risk_control_v2_preset(risk_preset)
