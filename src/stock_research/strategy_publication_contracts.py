"""Immutable registry and identity helpers for officially publishable strategies."""
...
class StrategyPublicationContract:
    ...
    def __post_init__(self) -> None:
        object.__setattr__(self, "normalized_run_config", _freeze_json(self.normalized_run_config))
...
def validate_publication_identity(
    ...
    fields = dict.fromkeys((*_IDENTITY_FIELDS, *expected.keys(), *actual.keys()))
    for field in fields:
...
_PUBLICATION_CONTRACTS: Mapping[tuple[str, str], StrategyPublicationContract] = MappingProxyType(
    {
...
OFFICIAL_STRATEGY_IDS = frozenset(strategy_id for strategy_id, _profile in _PUBLICATION_CONTRACTS)


def get_publication_contract(
    strategy_id: str, profile: str = "balanced"
) -> StrategyPublicationContract:
    try:
