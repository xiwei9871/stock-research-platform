...
def load_shadow_follow_up_resolution_read_model_rows(path: str | Path) -> dict[str, Any]:
    """Load one P18 JSON artifact into run and item read-model rows."""
    json_path = Path(path)
    resolution = json.loads(json_path.read_text(encoding="utf-8"))
    if not isinstance(resolution, dict):
        raise ValueError(f"operator shadow follow-up resolution must be a JSON object: {json_path}")

    _validate_safety_fields(resolution)

    run_id = _required_text(resolution, "run_id")
    resolution_date = _required_text(resolution, "resolution_date")
    items = [item for item in resolution.get("items", []) if isinstance(item, dict)]
    ...
    return {
        "run": {
            ...
            "status": _required_text(resolution, "status"),
            "operator_id": _required_text(resolution, "operator_id"),
            "source_p17_follow_up_run_ids": resolution.get("source_p17_follow_up_run_ids") or [],
            "item_count": int(resolution.get("item_count") or len(items)),
        ...
        "items": [
            _item_row(
                ...
                run_id=run_id,
                resolution_date=resolution_date,
                resolution_artifact_path=json_path,
            )
            for item in items
        ],
    }
...
def import_shadow_follow_up_resolution(
    path: str | Path,
    *,
    service: str = SETTINGS.research_service,
) -> dict[str, Any]:
    """Import one artifact or a directory of P18 artifacts into ops tables."""
    input_path = Path(path)
    paths = _resolution_paths(input_path)
    run_ids: list[str] = []
    item_count = 0
    with connect(service) as conn:
        with conn.cursor() as cur:
            for resolution_path in paths:
                rows = load_shadow_follow_up_resolution_read_model_rows(resolution_path)
...
def _resolution_paths(path: Path) -> list[Path]:
    if path.is_dir():
        return sorted(path.glob("operator_shadow_follow_up_resolution_*.json"))
    return [path]


def _upsert_run(cur: Any, row: dict[str, Any]) -> None:
    sql = """
    INSERT INTO ops.operator_shadow_follow_up_resolution_run (
        run_id, resolution_date, status, operator_id, source_p17_follow_up_run_ids,
    ...
    VALUES (
        %(run_id)s, %(resolution_date)s, %(status)s, %(operator_id)s,
        %(source_p17_follow_up_run_ids)s::jsonb, %(item_count)s,
    ...
    DO UPDATE SET
        ...
        source_p17_follow_up_run_ids = EXCLUDED.source_p17_follow_up_run_ids,
    ...
    cur.execute(
        ...
        {
            **row,
            "source_p17_follow_up_run_ids": json.dumps(
                row.get("source_p17_follow_up_run_ids") or [],
                sort_keys=True,
            ),
...
def _upsert_item(cur: Any, row: dict[str, Any]) -> None:
    sql = """
    INSERT INTO ops.operator_shadow_follow_up_resolution_item (
        resolution_item_id, run_id, resolution_date, source_p17_follow_up_item_id,
        source_p17_follow_up_run_id, source_p16_decision_group_id,
        ...
        priority_bucket, required_input, resolution_status, resolution_bucket,
        recommended_resolution_action, resolution_reason, follow_up_reason,
        decision_reason, required_next_action, evidence_summary, risk_notes,
        next_research_question, resolution_artifact_path, manual_review_required,
        auto_trade_enabled, production_watchlist_enabled, production_write_enabled,
        metadata
    )
    VALUES (
        %(resolution_item_id)s, %(run_id)s, %(resolution_date)s,
        %(source_p17_follow_up_item_id)s, %(source_p17_follow_up_run_id)s,
        ...
        %(resolution_status)s, %(resolution_bucket)s,
        %(recommended_resolution_action)s, %(resolution_reason)s,
        %(follow_up_reason)s, %(decision_reason)s, %(required_next_action)s,
        %(evidence_summary)s, %(risk_notes)s, %(next_research_question)s,
        %(resolution_artifact_path)s, %(manual_review_required)s,
        %(auto_trade_enabled)s, %(production_watchlist_enabled)s,
        %(production_write_enabled)s, %(metadata)s::jsonb
    )
    ON CONFLICT (resolution_item_id)
    DO UPDATE SET
        run_id = EXCLUDED.run_id,
        resolution_date = EXCLUDED.resolution_date,
        source_p17_follow_up_item_id = EXCLUDED.source_p17_follow_up_item_id,
        source_p17_follow_up_run_id = EXCLUDED.source_p17_follow_up_run_id,
        ...
        resolution_bucket = EXCLUDED.resolution_bucket,
        recommended_resolution_action = EXCLUDED.recommended_resolution_action,
        ...
        resolution_artifact_path = EXCLUDED.resolution_artifact_path,
        manual_review_required = EXCLUDED.manual_review_required,
        auto_trade_enabled = EXCLUDED.auto_trade_enabled,
        production_watchlist_enabled = EXCLUDED.production_watchlist_enabled,
        production_write_enabled = EXCLUDED.production_write_enabled,
...
def _item_row(
    ...
    *,
    ...
    source_follow_up_item_id = _required_text(item, "source_p17_follow_up_item_id")
    resolution_status = _required_text(item, "resolution_status")
    return {
        "resolution_item_id": _resolution_item_id(
            run_id=run_id,
            source_p17_follow_up_item_id=source_follow_up_item_id,
            resolution_status=resolution_status,
        ),
        "run_id": run_id,
        "resolution_date": resolution_date,
        "source_p17_follow_up_item_id": source_follow_up_item_id,
        "source_p17_follow_up_run_id": _required_text(item, "source_p17_follow_up_run_id"),
        "source_p16_decision_group_id": _required_text(item, "source_p16_decision_group_id"),
        ...
        "resolution_status": resolution_status,
        "resolution_bucket": _required_text(item, "resolution_bucket"),
        "recommended_resolution_action": str(item.get("recommended_resolution_action") or ""),
        ...
        "resolution_artifact_path": str(resolution_artifact_path),
        "manual_review_required": True,
        "auto_trade_enabled": False,
        "production_watchlist_enabled": False,
        "production_write_enabled": False,
...
def _resolution_item_id(*, run_id: str, source_p17_follow_up_item_id: str, resolution_status: str) -> str:
    raw = "|".join([run_id, source_p17_follow_up_item_id, resolution_status])
    digest = hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]
    return f"operator_shadow_follow_up_resolution:{run_id}:{digest}"
