...
TASK_NAME = "tech_bottleneck_latent_manual_review_standard_backfill_v1"
...
EXPECTED_STOCK_COUNT = 52
EXPECTED_PDF_COUNT = 156
...
def _load_cached(output: Path) -> dict[str, Any] | None:
    summary_path = output / "latent_manual_review_standard_backfill_summary.json"
    required = [
        summary_path,
        output / "latent_manual_review_standard_backfill_evidence.csv",
        output / "latent_manual_review_standard_backfill_stock_status.csv",
        output / "latent_manual_review_standard_backfill_page_citations.csv",
        output / "latent_manual_review_standard_backfill_parse_gaps.csv",
        output / "latent_manual_review_standard_backfill_guardrails.json",
        output / "tech_bottleneck_latent_manual_review_standard_backfill_v1_report.md",
    ]
    if not all(path.exists() for path in required):
        return None
    summary = _read_json(summary_path)
    if (
        summary.get("source_standard_collection_stock_count") == EXPECTED_STOCK_COUNT
        and summary.get("source_standard_collection_pdf_count") == EXPECTED_PDF_COUNT
...
def _parse_sources(
    ...
    *,
    ...
    chunks_path = output / "latent_manual_review_standard_backfill_text_first_chunks.csv"
    parse_manifest_path = output / "latent_manual_review_standard_backfill_text_first_parse_manifest.csv"
...
def _status_from_counts(stock_evidence: pd.DataFrame) -> tuple[str, str, str]:
    ...
    if primary_count and page_count and hard_hits >= 2 and value_hits >= 1:
        return (
            ...
            "proceed to latent manual review standard equivalence gate; no automatic quality-pool action",
        )
    if primary_count and (hard_hits or architecture_hits or value_hits):
        return (
            ...
            "manual review remaining evidence gaps before standard equivalence gate",
        )
    return (
        ...
        rows.append(
            {
                ...
                "gap_detail": "no downloaded primary-source PDF found in standard collection manifest",
...
def _summary(
    *,
    ...
    has_partial = bool(
        int(status_counts.get("partially_supported", 0))
        or int(status_counts.get("insufficient_primary_source_evidence", 0))
        or int(status_counts.get("parse_failed_or_unusable", 0))
    )
    if blocking:
        ...
        acceptance = "conditionally_ready_with_partial_support"
    else:
        acceptance = "latent_manual_review_standard_backfill_ready"
    return {
        "task_name": TASK_NAME,
        "research_only": True,
        "source_standard_collection_stock_count": int(len(queue)),
        "source_standard_collection_pdf_count": int(len(sources)),
...
def _guardrails(summary: dict[str, Any]) -> dict[str, Any]:
    return {
        "task_name": TASK_NAME,
        "research_only": True,
        "source_standard_collection_stock_count": summary["source_standard_collection_stock_count"],
        "source_standard_collection_pdf_count": summary["source_standard_collection_pdf_count"],
...
def _report(summary: dict[str, Any]) -> str:
    return "\n".join(
        [
            "# Tech Bottleneck Latent Manual Review Standard Backfill v1",
            "",
            "## 1. Scope",
            "This task processes only the 52-stock latent manual-review standard collection batch and its 156 downloaded primary-source PDFs. It performs PDF parsing, evidence extraction, page-level citation generation, and backfill status classification only.",
            "",
...
def run(output_dir: str | Path = OUTPUT_DIR, *, force: bool = False) -> dict[str, Any]:
    ...
    evidence.to_csv(output / "latent_manual_review_standard_backfill_evidence.csv", index=False)
    stock_status.to_csv(output / "latent_manual_review_standard_backfill_stock_status.csv", index=False)
    citations.to_csv(output / "latent_manual_review_standard_backfill_page_citations.csv", index=False)
    parse_gaps.to_csv(output / "latent_manual_review_standard_backfill_parse_gaps.csv", index=False)
    _write_json(output / "latent_manual_review_standard_backfill_summary.json", summary)
    _write_json(output / "latent_manual_review_standard_backfill_guardrails.json", guardrails)
    (output / "tech_bottleneck_latent_manual_review_standard_backfill_v1_report.md").write_text(
        _report(summary),
        encoding="utf-8",
    )
    return summary
