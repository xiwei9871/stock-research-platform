...
TASK_NAME = "tech_bottleneck_latent_manual_review_standard_equivalence_gate_v1"
...
EXPECTED_COUNT = 52
EXPECTED_EVIDENCE_ROWS = 1096
EXPECTED_PAGE_CITATIONS = 1096
...
DECISION_COLUMNS = [
    ...
    "quality_pool_v6_proposal_processed",
    "frozen_quality_pool_v6_generated",
    "new_quality_pool_layer_generated",
...
def _load_cached(output: Path) -> dict[str, Any] | None:
    summary_path = output / "latent_manual_review_standard_equivalence_gate_summary.json"
    required = [
        summary_path,
        output / "latent_manual_review_standard_equivalence_gate_decisions.csv",
        output / "latent_manual_review_standard_equivalence_gate_core_equivalent_proposals.csv",
        output / "latent_manual_review_standard_equivalence_gate_keep_separate.csv",
        output / "latent_manual_review_standard_equivalence_gate_human_confirm_required.csv",
        output / "latent_manual_review_standard_equivalence_gate_downgrade_or_reject.csv",
        output / "latent_manual_review_standard_equivalence_gate_guardrails.json",
        output / "tech_bottleneck_latent_manual_review_standard_equivalence_gate_v1_report.md",
    ]
    if not all(path.exists() for path in required):
        return None
    summary = _read_json(summary_path)
    if (
        summary.get("source_standard_backfill_stock_count") == EXPECTED_COUNT
        and summary.get("processed_stock_count") == EXPECTED_COUNT
        and summary.get("source_evidence_rows") == EXPECTED_EVIDENCE_ROWS
        and summary.get("source_page_level_citations") == EXPECTED_PAGE_CITATIONS
    ):
        return summary
    return None
...
def _decision(row: pd.Series) -> tuple[str, str]:
    ...
    if "missing_business_or_financial_trace" in gaps or "missing_hard_tech_domain_evidence" in gaps:
        return "keep_separate_latent_candidate", "business or hard-tech evidence gap prevents quality-pool v5/v6 proposal equivalence"
    score = _int(row.get("standard_equivalence_score"))
    if score >= 14:
        return "core_equivalent_proposal", "page-level primary-source evidence meets quality pool v5/v6 proposal equivalence threshold"
    if score >= 10:
    ...
    return "downgrade_or_reject", "primary-source evidence is too weak for latent standard continuation"


def _build_decisions(status: pd.DataFrame, queue: pd.DataFrame) -> pd.DataFrame:
    merge_columns = ["stock_code"]
    for column in ["needs_human_supply_chain_role_confirmation", "triage_reason", "recommended_next_action"]:
        if column in queue.columns:
            merge_columns.append(column)
    frame = status.merge(queue[merge_columns].copy(), on="stock_code", how="left")
    rows: list[dict[str, Any]] = []
    for _, source_row in frame.sort_values("stock_code").iterrows():
        score = _score(source_row)
        row = source_row.copy()
        row["standard_equivalence_score"] = score
        ...
        rows.append(
            {
                ...
                "standard_equivalence_score": score,
                ...
                "quality_pool_v6_proposal_processed": False,
                "frozen_quality_pool_v6_generated": False,
                "new_quality_pool_layer_generated": False,
                ...
                "notes": "Standard equivalence proposal only; no quality-pool layer generation or signal/admission use.",
            }
        )
...
def _summary(
    *,
    ...
    v5_overlap = len(set(decisions["stock_code"]) & set(quality_pool_v5["stock_code"])) if not decisions.empty else 0
    v6_proposal_overlap = len(set(decisions["stock_code"]) & set(v6_proposal["stock_code"])) if not decisions.empty else 0
    v6_manual_overlap = len(set(decisions["stock_code"]) & set(v6_manual["stock_code"])) if not decisions.empty else 0
    source_evidence_rows = int(len(evidence))
    source_page_citations = int(len(citations))
    blocking = (
        int(backfill_summary.get("processed_stock_count", 0)) != EXPECTED_COUNT
        or int(backfill_summary.get("primary_source_supported_count", 0)) != EXPECTED_COUNT
        or source_evidence_rows != EXPECTED_EVIDENCE_ROWS
        or source_page_citations != EXPECTED_PAGE_CITATIONS
        or len(decisions) != EXPECTED_COUNT
        or source_supported != EXPECTED_COUNT
        or v5_overlap != 0
        or v6_proposal_overlap != 0
        or v6_manual_overlap != 0
        or used_for_signal
    ...
    if blocking:
        ...
        acceptance = "latent_manual_review_standard_equivalence_gate_ready"
    return {
        "task_name": TASK_NAME,
        "research_only": True,
        "source_standard_backfill_stock_count": int(len(status)),
        ...
        "source_page_level_citations": source_page_citations,
        "quality_pool_v5_reference_count": int(len(quality_pool_v5)),
        "quality_pool_v5_overlap_count": int(v5_overlap),
        "quality_pool_v6_proposal_reference_count": int(len(v6_proposal)),
        "quality_pool_v6_proposal_overlap_count": int(v6_proposal_overlap),
        "quality_pool_v6_manual_reference_count": int(len(v6_manual)),
        "quality_pool_v6_manual_overlap_count": int(v6_manual_overlap),
...
def _guardrails(summary: dict[str, Any]) -> dict[str, Any]:
    return {
        "task_name": TASK_NAME,
        "research_only": True,
        "source_standard_backfill_stock_count": summary["source_standard_backfill_stock_count"],
        ...
        "source_evidence_rows": summary["source_evidence_rows"],
        "source_page_level_citations": summary["source_page_level_citations"],
        "core_equivalence_performed": True,
        "primary_source_collection_performed": False,
        "evidence_backfill_performed": False,
        "new_pdf_download_count": 0,
...
def _report(summary: dict[str, Any]) -> str:
    return "\n".join(
        [
            "# Tech Bottleneck Latent Manual Review Standard Equivalence Gate v1",
            "",
            "## 1. Scope",
            "This task evaluates only the 52 primary-source-supported standard backfill stocks against the quality pool v5 and v6 proposal standard. It does not collect PDFs, regenerate backfill evidence, process quality pools, generate a new layer, or connect to signal/admission/scoring/strategy.",
            "",
            "## 2. Input Backfill Baseline",
            f"Source stocks: {summary['source_standard_backfill_stock_count']}; primary-source supported: {summary['source_primary_source_supported_count']}; evidence rows: {summary['source_evidence_rows']}; page-level citations: {summary['source_page_level_citations']}.",
            "",
...
def run(output_dir: str | Path = OUTPUT_DIR, *, force: bool = False) -> dict[str, Any]:
    ...
    v6_proposal = _read_csv(V6_PROPOSAL)
    v6_manual = _read_csv(V6_MANUAL)
    ...
    summary = _summary(
        ...
        evidence=evidence,
        citations=citations,
        quality_pool_v5=quality_pool_v5,
        v6_proposal=v6_proposal,
        v6_manual=v6_manual,
    ...
    decisions.to_csv(output / "latent_manual_review_standard_equivalence_gate_decisions.csv", index=False)
    ...
    _write_json(output / "latent_manual_review_standard_equivalence_gate_summary.json", summary)
    _write_json(output / "latent_manual_review_standard_equivalence_gate_guardrails.json", guardrails)
    (output / "tech_bottleneck_latent_manual_review_standard_equivalence_gate_v1_report.md").write_text(
        _report(summary),
        encoding="utf-8",
    )
    return summary
