...
TASK_NAME = "tech_bottleneck_latent_standard_core_equivalence_gate_v1"
INPUT_CANDIDATES = (
    PROJECT_ROOT
    / "outputs/research/tech_bottleneck_latent_standard_backfill_queue_v1/latent_standard_manual_approval_candidates.csv"
)
INPUT_EVIDENCE = (
    PROJECT_ROOT / "outputs/research/tech_bottleneck_latent_standard_backfill_queue_v1/latent_standard_evidence_matrix.csv"
)
...
OUTPUT_COLUMNS = [
    ...
    "latent_standard_core_equivalence_score",
...
def _decision(row: pd.Series) -> tuple[str, str]:
    ...
    if "missing_route_around" in gaps:
        return "keep_as_latent_standard_candidate", "route-around evidence gap prevents same-layer quality-pool equivalence"
    if "missing_financial_trace" in gaps or "missing_annual_report" in gaps:
        return "keep_as_latent_standard_candidate", "financial or annual-report evidence gap remains"
    score = _int(row.get("latent_standard_core_equivalence_score"))
    if score >= 13:
        return (
    ...
    if score >= 9:
        return (
            ...
            "primary-source evidence supports latent standard continuation but score is below quality-pool equivalence threshold",
        )
    if score >= 5:
        return "remain_latent_standard_watch", "evidence remains incomplete for equivalence and should stay on latent standard watch"
    return "downgrade_or_reject", "backfill evidence is too weak for latent standard continuation"
...
def _build_gate(candidates: pd.DataFrame, evidence: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in gate.sort_values("stock_code").iterrows():
        score = _score(row)
        row = row.copy()
        row["latent_standard_core_equivalence_score"] = score
        ...
        rows.append(
            {
                ...
                "latent_standard_core_equivalence_score": score,
...
def _split(gate: pd.DataFrame) -> dict[str, pd.DataFrame]:
    return {
        "latent_standard_core_equivalent_add_to_quality_pool": gate[
            gate["core_equivalence_decision"].eq("latent_standard_core_equivalent_add_to_quality_pool")
        ].copy(),
        "keep_as_latent_standard_candidate": gate[
            gate["core_equivalence_decision"].eq("keep_as_latent_standard_candidate")
        ].copy(),
        "remain_latent_standard_watch": gate[gate["core_equivalence_decision"].eq("remain_latent_standard_watch")].copy(),
        "downgrade_or_reject": gate[gate["core_equivalence_decision"].eq("downgrade_or_reject")].copy(),
    }


def _summary(gate: pd.DataFrame, quality_pool_v4: pd.DataFrame, strategy_clean: bool) -> dict[str, Any]:
    ...
    overlap = len(set(gate["stock_code"]) & set(quality_pool_v4["stock_code"]))
    blocking = (
        len(gate) != EXPECTED_COUNT
        or len(quality_pool_v4) != 276
        or overlap != 0
        or used_for_signal
    ...
    if blocking:
        ...
        counts.get("keep_as_latent_standard_candidate", 0)
        or counts.get("remain_latent_standard_watch", 0)
        or counts.get("downgrade_or_reject", 0)
    ):
        acceptance = "conditionally_ready_with_equivalence_gaps"
    else:
        acceptance = "latent_standard_core_equivalence_gate_ready"
    return {
        "task_name": TASK_NAME,
        "source_latent_standard_candidate_count": int(len(gate)),
        "processed_count": int(len(gate)),
        "quality_pool_v4_reference_count": int(len(quality_pool_v4)),
        "quality_pool_v4_processed": False,
        "quality_pool_v4_overlap_count": int(overlap),
        "latent_standard_core_equivalent_count": int(
            counts.get("latent_standard_core_equivalent_add_to_quality_pool", 0)
        ),
        "keep_as_latent_standard_candidate_count": int(counts.get("keep_as_latent_standard_candidate", 0)),
        "remain_latent_standard_watch_count": int(counts.get("remain_latent_standard_watch", 0)),
...
def _guardrails(summary: dict[str, Any]) -> dict[str, Any]:
    return {
        "task_name": TASK_NAME,
        "research_only": True,
        "source_latent_standard_candidate_count": summary["source_latent_standard_candidate_count"],
...
def _report(summary: dict[str, Any]) -> str:
    return "\n".join(
        [
            "# Tech Bottleneck Latent Standard Core Equivalence Gate v1",
            "",
            "## 1. Scope",
            "This task evaluates only the 24 latent standard manual approval candidates against the quality pool v4 standard. It does not process completed high-priority candidates, manual-review-first names, defer/reject names, or automatically add anything to a quality pool.",
            "",
            "## 2. Quality Pool v4 Reference",
            f"Quality pool v4 reference count: {summary['quality_pool_v4_reference_count']}; processed as input queue: {str(summary['quality_pool_v4_processed']).lower()}; overlap with latent standard candidates: {summary['quality_pool_v4_overlap_count']}.",
            "",
            "## 3. Equivalence Results",
            f"Processed: {summary['processed_count']}; core-equivalent proposals: {summary['latent_standard_core_equivalent_count']}; keep separate: {summary['keep_as_latent_standard_candidate_count']}; remain watch: {summary['remain_latent_standard_watch_count']}; downgrade/reject: {summary['downgrade_or_reject_count']}.",
            "",
            "## 4. Guardrails",
            ...
            "1. tech_bottleneck_quality_pool_layer_v5",
            "2. tech_bottleneck_latent_manual_review_first_triage_v1",
...
def run(output_dir: str | Path = OUTPUT_DIR) -> dict[str, Any]:
    ...
    quality_pool_v4 = _read_csv(QUALITY_POOL_V4)
    ...
    summary = _summary(gate, quality_pool_v4, strategy_clean)
    guardrails = _guardrails(summary)

    gate.to_csv(output / "latent_standard_core_equivalence_gate.csv", index=False)
    splits["latent_standard_core_equivalent_add_to_quality_pool"].to_csv(
        output / "latent_standard_core_equivalent_candidates.csv", index=False
    )
    splits["keep_as_latent_standard_candidate"].to_csv(
        output / "latent_standard_keep_separate_candidates.csv", index=False
    )
    splits["remain_latent_standard_watch"].to_csv(output / "latent_standard_remain_watch.csv", index=False)
    splits["downgrade_or_reject"].to_csv(output / "latent_standard_downgrade_or_reject.csv", index=False)
    _write_json(output / "latent_standard_core_equivalence_summary.json", summary)
    _write_json(output / "latent_standard_core_equivalence_guardrails.json", guardrails)
    (output / "tech_bottleneck_latent_standard_core_equivalence_gate_v1_report.md").write_text(
        _report(summary),
        encoding="utf-8",
    )
    return summary
