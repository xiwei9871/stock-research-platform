...
def compute_price_features(
    ...
    *,
    ...
    insufficient = ~result["relative_return_coverage"]
    relative_columns = [
        "industry_return_6m",
        "relative_return_6m",
        "industry_return_60d",
    ...
    result.loc[insufficient, relative_columns] = np.nan
...
def compute_oversold_score(frame: pd.DataFrame) -> pd.Series:
    ...
    ma_distance = numeric[["distance_ma120", "distance_ma250"]].mean(axis=1, skipna=True)
...
def compute_already_priced_features(
    ...
    *,
    ...
    raw_valuation = valuation_row.get("valuation_percentile", math.nan)
    if pd.isna(raw_valuation):
        valuation, valuation_coverage = math.nan, False
    else:
        try:
            valuation = float(raw_valuation)
        except (TypeError, ValueError) as exc:
            raise ValueError("valuation_percentile must be between 0 and 1") from exc
        if not math.isfinite(valuation) or not 0.0 <= valuation <= 1.0:
            raise ValueError("valuation_percentile must be between 0 and 1")
        valuation_coverage = True
