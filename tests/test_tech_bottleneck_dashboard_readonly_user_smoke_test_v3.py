...
OUTPUT_DIR = PROJECT_ROOT / "outputs/research/tech_bottleneck_dashboard_readonly_user_smoke_test_v3"
...
def test_smoke_v3_outputs_summary_and_section_status_are_valid() -> None:
    expected = {
        "smoke_test_v3_summary.json",
        "smoke_test_v3_section_status.csv",
        "smoke_test_v3_financial_statement_section_checks.json",
        "smoke_test_v3_route_nav_checks.json",
        "smoke_test_v3_data_consistency_checks.csv",
        "smoke_test_v3_guardrail_checks.json",
        "smoke_test_v3_test_results.json",
        "tech_bottleneck_dashboard_readonly_user_smoke_test_v3_report.md",
    }
    assert OUTPUT_DIR.exists()
    assert expected.issubset({path.name for path in OUTPUT_DIR.iterdir()})
    summary = json.loads((OUTPUT_DIR / "smoke_test_v3_summary.json").read_text(encoding="utf-8"))
    sections = pd.read_csv(OUTPUT_DIR / "smoke_test_v3_section_status.csv")
    assert summary["acceptance_decision"] == "dashboard_readonly_internal_review_ready_with_financial_statement_context"
    ...
    assert summary["sections_passed"] == 9
    assert summary["sections_partial"] == 0
    assert summary["sections_failed"] == 0
    assert len(sections) == 9
    assert set(sections["v3_status"]) == {"passed"}
    assert "Full Financial Statement Review Context" in set(sections["section_name"])


def test_smoke_v3_financial_statement_counts_and_data_consistency_match() -> None:
    summary = json.loads((OUTPUT_DIR / "smoke_test_v3_summary.json").read_text(encoding="utf-8"))
    financial = json.loads((OUTPUT_DIR / "smoke_test_v3_financial_statement_section_checks.json").read_text(encoding="utf-8"))
    consistency = pd.read_csv(OUTPUT_DIR / "smoke_test_v3_data_consistency_checks.csv")
    ...
    assert financial["section_status"] == "passed"
    assert financial["supported_count"] == 63
    assert financial["missing_count"] == 39
    assert financial["missing_rows_have_data_gap_note"] is True
    assert set(consistency["status"]) == {"passed"}


def test_smoke_v3_guardrails_frontend_and_formal_strategy_are_clean() -> None:
    summary = json.loads((OUTPUT_DIR / "smoke_test_v3_summary.json").read_text(encoding="utf-8"))
    route_nav = json.loads((OUTPUT_DIR / "smoke_test_v3_route_nav_checks.json").read_text(encoding="utf-8"))
    guardrails = json.loads((OUTPUT_DIR / "smoke_test_v3_guardrail_checks.json").read_text(encoding="utf-8"))
