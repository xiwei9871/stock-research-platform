...
def test_get_latest_indicator_rows_batches_assets_with_announcement_date_cutoff(monkeypatch):
    conn = FakeConnection(
        [
            {
                "asset_id": "CN:SH:600000",
    ...
    rows = point_in_time_finance.get_latest_indicator_rows(
        conn,
    ...
    assert rows == {
        "CN:SH:600000": conn.rows[0],
        "CN:SZ:000001": conn.rows[1],
    }
    sql, params = conn.calls[0]
    assert "FROM finance.indicator_quarter" in sql
    ...
    assert "announcement_date <= %s" in sql
    assert "ORDER BY asset_id, announcement_date DESC, report_period DESC" in sql
    assert params == [["CN:SH:600000", "CN:SZ:000001"], "2026-04-01"]


def test_get_latest_indicator_rows_skips_query_without_assets(monkeypatch):
    conn = FakeConnection([])
    monkeypatch.setattr(point_in_time_finance, "fetch_all", fake_fetch_all)

    rows = point_in_time_finance.get_latest_indicator_rows(conn, [], "2026-04-01")
...
def test_batched_statement_helpers_query_standardized_finance_tables(monkeypatch):
    conn = FakeConnection([{"asset_id": "CN:SH:600000"}])
    monkeypatch.setattr(point_in_time_finance, "fetch_all", fake_fetch_all)

    point_in_time_finance.get_latest_income_statement_rows(
        conn,
        ["CN:SH:600000"],
        "2026-04-01",
    )
    ...
    point_in_time_finance.get_latest_cash_flow_rows(
        conn,
        ["CN:SH:600000"],
        "2026-04-01",
    )
...
def test_get_latest_share_capital_event_rows_uses_pit_event_cutoffs(monkeypatch):
    conn = FakeConnection(
        [
            {
                "asset_id": "CN:SH:600000",
    ...
    rows = point_in_time_finance.get_latest_share_capital_event_rows(
        conn,
        ["CN:SH:600000", "CN:SZ:000001"],
        "2026-04-01",
    )
    ...
    sql, params = conn.calls[0]
    assert "FROM finance.share_capital_event" in sql
    assert "DISTINCT ON (asset_id)" in sql
    assert "asset_id = ANY(%s)" in sql
    assert "event_date <= %s" in sql
    assert "(announcement_date IS NULL OR announcement_date <= %s)" in sql
    assert "ORDER BY asset_id, event_date DESC" in sql
    assert params == [["CN:SH:600000", "CN:SZ:000001"], "2026-04-01", "2026-04-01"]
