...
def test_ai_power_company_mapping_package_loads_and_summarizes():
    ...
    assert summary == {
        ...
        "evidence_count": 8,
        "mapping_count": 4,
        "mappings_by_business_stage": {"primary_business": 4},
        "mappings_by_node": {
            "hvdc_power": 1,
            "liquid_cooling": 1,
        ...
        "mappings_by_review_status": {"reviewed": 4},
        "source_count": 4,
        "theme_count": 1,
    }
...
def test_reviewed_mapping_allows_weaker_supplemental_source(tmp_path: Path):
    ...
    assert len(package["company_mappings"]) == 4
...
def test_cli_validate_summary_and_company_lookup_emit_json(capsys):
    ...
    assert validate_payload["mapping_count"] == 4
    assert summary_exit == 0
    assert summary_payload["company_count"] == 4
