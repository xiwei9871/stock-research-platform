...
def test_industry_exposure_risk_control_cli_prints_outputs(monkeypatch, capsys):
    ...
    monkeypatch.setattr(cli, "run_industry_exposure_risk_control", fake_runner)
    monkeypatch.setattr(
        "sys.argv",
        [
            "stock-research",
    ...
    assert "industry_exposure_risk_control|summary|/tmp/summary.csv" in out
    assert "industry_exposure_risk_control|markdown_report|/tmp/report.md" in out
