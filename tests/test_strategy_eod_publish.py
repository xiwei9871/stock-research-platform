...
def test_publish_strategy_eod_score_audit_failure_is_loadable_from_disk(monkeypatch, tmp_path):
    ...
    assert "detail_path" not in loaded


def test_publish_strategy_eod_score_audit_double_failure_still_returns_failed_summary(monkeypatch, tmp_path):
    monkeypatch.setattr(strategy_eod_publish, "_ensure_strategy_dependencies", lambda *args, **kwargs: None)
    monkeypatch.setattr(
        strategy_eod_publish,
        "_build_base_manifest_entries",
    ...
    monkeypatch.setattr(
        strategy_eod_publish,
        "_write_strategy_score_audit_artifacts",
        lambda **kwargs: (_ for _ in ()).throw(RuntimeError("primary audit write failed")),
    )
    monkeypatch.setattr(
        strategy_eod_publish,
        "_write_strategy_score_audit_failure_summary",
        lambda **kwargs: (_ for _ in ()).throw(RuntimeError("fallback audit summary write failed")),
    )
    ...
    assert summary["score_audit"]["trade_date"] == "2026-06-22"
    assert summary["score_audit"]["status"] == "failed"
    assert summary["score_audit"]["error"] == "primary audit write failed"


def test_publish_strategy_eod_score_audit_double_failure_is_loadable_from_publish_summary(monkeypatch, tmp_path):
    monkeypatch.setattr(strategy_eod_publish, "_ensure_strategy_dependencies", lambda *args, **kwargs: None)
    monkeypatch.setattr(
        strategy_eod_publish,
        "_build_base_manifest_entries",
