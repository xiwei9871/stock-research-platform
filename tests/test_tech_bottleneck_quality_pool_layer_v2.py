...
SCRIPT = PROJECT_ROOT / "scripts/run_tech_bottleneck_quality_pool_layer_v2.py"
...
OUTPUT_DIR = PROJECT_ROOT / "outputs/research/tech_bottleneck_quality_pool_layer_v2"
...
def test_quality_pool_layer_v2_outputs_and_guardrails() -> None:
    v1_hash_before = _sha(V1_MANIFEST)
    rescue_hash_before = _sha(RESCUE_38)
    _run_generator()
    v1_hash_after = _sha(V1_MANIFEST)
    rescue_hash_after = _sha(RESCUE_38)

    expected = {
        "quality_pool_layer_v2_manifest.csv",
        "quality_pool_layer_v2_summary.json",
        "quality_pool_layer_v2_by_source.csv",
        ...
        "quality_pool_layer_v2_guardrails.json",
        "tech_bottleneck_quality_pool_layer_v2_report.md",
    }
    assert OUTPUT_DIR.exists()
    assert expected.issubset({path.name for path in OUTPUT_DIR.iterdir()})
    assert v1_hash_before == v1_hash_after
    assert rescue_hash_before == rescue_hash_after

    summary = json.loads((OUTPUT_DIR / "quality_pool_layer_v2_summary.json").read_text(encoding="utf-8"))
    guardrails = json.loads((OUTPUT_DIR / "quality_pool_layer_v2_guardrails.json").read_text(encoding="utf-8"))

    assert summary["quality_pool_v1_count"] == 172
    ...
    assert summary["acceptance_decision"] == "quality_pool_layer_v2_ready"

    assert guardrails["research_only"] is True
    assert guardrails["quality_pool_layer_v2_generated"] is True
    assert guardrails["quality_pool_v2_count"] == 210
...
def test_quality_pool_layer_v2_manifest_integrity_and_sidecars() -> None:
    _run_generator()

    manifest = pd.read_csv(OUTPUT_DIR / "quality_pool_layer_v2_manifest.csv", dtype={"stock_code": str})
    by_source = pd.read_csv(OUTPUT_DIR / "quality_pool_layer_v2_by_source.csv")
    expansion_keep = pd.read_csv(OUTPUT_DIR / "expansion_keep_separate_4.csv", dtype={"stock_code": str})
    rescue_keep = pd.read_csv(OUTPUT_DIR / "rescue_keep_separate_1.csv", dtype={"stock_code": str})
    reject = pd.read_csv(OUTPUT_DIR / "downgrade_or_reject_2.csv", dtype={"stock_code": str})
    possible = pd.read_csv(OUTPUT_DIR / "possible_false_negative_manual_review_9.csv", dtype={"stock_code": str})
    remain = pd.read_csv(OUTPUT_DIR / "remain_excluded_22.csv", dtype={"stock_code": str})
    concept = pd.read_csv(OUTPUT_DIR / "reject_concept_or_non_bottleneck_6.csv", dtype={"stock_code": str})

    assert len(manifest) == 210
    assert manifest["stock_code"].nunique() == 210
    ...
    assert "688813" not in set(manifest["stock_code"])
    assert set(rescue_keep["stock_code"]) == {"688813"}
    assert rescue_keep["remaining_evidence_gap_flags"].str.contains("missing_architecture_shift").all()
    assert rescue_keep["remaining_evidence_gap_flags"].str.contains("missing_route_around").all()

    assert len(expansion_keep) == 4
    assert len(rescue_keep) == 1
    assert len(reject) == 2
    assert len(possible) == 9
    assert len(remain) == 22
    assert len(concept) == 6
    for sidecar in [expansion_keep, rescue_keep, reject, possible, remain, concept]:
...
def test_quality_pool_layer_v2_deterministic_and_strategy_diff_clean() -> None:
    _run_generator()
