...
SCRIPT = PROJECT_ROOT / "scripts/run_tech_bottleneck_quality_pool_layer_v6_proposal.py"
...
OUTPUT_DIR = PROJECT_ROOT / "outputs/research/tech_bottleneck_quality_pool_layer_v6_proposal_v1"
...
def test_quality_pool_layer_v6_proposal_outputs_and_guardrails() -> None:
    input_hashes_before = {
        "v5": _sha(V5_MANIFEST),
    ...
    expected = {
        "tech_bottleneck_quality_pool_layer_v6_proposal_summary.json",
        "tech_bottleneck_quality_pool_layer_v6_proposal.csv",
        "tech_bottleneck_quality_pool_layer_v6_added_from_batch1.csv",
        "tech_bottleneck_quality_pool_layer_v6_duplicate_check.csv",
        "tech_bottleneck_quality_pool_layer_v6_evidence_index.csv",
        "tech_bottleneck_quality_pool_layer_v6_guardrails.json",
        "tech_bottleneck_quality_pool_layer_v6_proposal_v1_report.md",
    }
    ...
    summary = json.loads(
        (OUTPUT_DIR / "tech_bottleneck_quality_pool_layer_v6_proposal_summary.json").read_text(encoding="utf-8")
    )
    guardrails = json.loads(
        (OUTPUT_DIR / "tech_bottleneck_quality_pool_layer_v6_guardrails.json").read_text(encoding="utf-8")
    )

    assert summary["research_only"] is True
    assert summary["quality_pool_v5_reference_count"] == 300
    assert summary["source_core_equivalent_proposal_count"] == 26
    assert summary["processed_core_equivalent_proposal_count"] == 26
    assert summary["duplicate_stock_count"] == 0
    assert summary["proposed_addition_count"] == 26
    assert summary["proposed_quality_pool_v6_count"] == 326
    ...
    assert summary["quality_pool_v6_is_proposal_only"] is True
    ...
    assert summary["acceptance_decision"] == "quality_pool_layer_v6_proposal_ready"

    assert guardrails["research_only"] is True
    assert guardrails["quality_pool_v5_reference_count"] == 300
    assert guardrails["source_core_equivalent_proposal_count"] == 26
    assert guardrails["processed_core_equivalent_proposal_count"] == 26
    assert guardrails["duplicate_stock_count"] == 0
    assert guardrails["proposed_addition_count"] == 26
    assert guardrails["proposed_quality_pool_v6_count"] == 326
    assert guardrails["quality_pool_v6_is_proposal_only"] is True
...
def test_quality_pool_layer_v6_proposal_manifest_and_added_rows() -> None:
    _run_generator()

    v5 = pd.read_csv(V5_MANIFEST, dtype={"stock_code": str})
    gate_core = pd.read_csv(GATE_CORE, dtype={"stock_code": str})
    proposal = pd.read_csv(OUTPUT_DIR / "tech_bottleneck_quality_pool_layer_v6_proposal.csv", dtype={"stock_code": str})
    added = pd.read_csv(OUTPUT_DIR / "tech_bottleneck_quality_pool_layer_v6_added_from_batch1.csv", dtype={"stock_code": str})
    duplicate = pd.read_csv(OUTPUT_DIR / "tech_bottleneck_quality_pool_layer_v6_duplicate_check.csv", dtype={"stock_code": str})
    evidence_index = pd.read_csv(
        OUTPUT_DIR / "tech_bottleneck_quality_pool_layer_v6_evidence_index.csv",
        dtype={"stock_code": str},
    )

    assert len(v5) == 300
    assert len(gate_core) == 26
    assert len(proposal) == 326
    assert proposal["stock_code"].nunique() == 326
    assert set(v5["stock_code"].astype(str).str.zfill(6)).issubset(set(proposal["stock_code"].astype(str).str.zfill(6)))
    ...
    assert added["source_layer"].eq("latent_manual_review_batch1_core_equivalent_proposal").all()
    assert added["v6_proposal_status"].eq("proposed_addition_only").all()
    assert added["added_from"].eq("latent_manual_review_equivalence_gate_batch1_v1").all()
...
def test_quality_pool_layer_v6_proposal_deterministic_and_strategy_diff_clean() -> None:
    _run_generator()
