...
from stock_research.dashboard.shadow_follow_up_resolution import (
...
def test_load_shadow_follow_up_resolution_summary_queries_items_and_forces_safety(monkeypatch):
    captured = {}
    ...
    monkeypatch.setattr(
        "stock_research.dashboard.shadow_follow_up_resolution.connect",
        fake_connect,
    )
    monkeypatch.setattr(
        "stock_research.dashboard.shadow_follow_up_resolution.fetch_all",
        fake_fetch_all,
    )

    rows = load_shadow_follow_up_resolution_summary(
        "2026-06-01",
        "2026-08-31",
        limit=10,
    ...
    assert "FROM ops.operator_shadow_follow_up_resolution_item" in captured["sql"]
    assert "resolution_date BETWEEN %s AND %s" in captured["sql"]
    assert captured["params"] == ["2026-06-01", "2026-08-31", 10]
    assert captured["service"] == "test-service"
    assert rows[0]["resolution_status"] == "stale_unresolved"
    assert rows[0]["resolution_bucket"] == "needs_operator_review"
    assert rows[0]["recommended_resolution_action"] == "Review whether requested evidence has been collected."
...
def test_load_shadow_follow_up_resolution_summary_returns_empty_for_missing_table(monkeypatch):
    monkeypatch.setattr(
        "stock_research.dashboard.shadow_follow_up_resolution.connect",
    ...
    assert load_shadow_follow_up_resolution_summary("2026-06-01", "2026-08-31") == []
