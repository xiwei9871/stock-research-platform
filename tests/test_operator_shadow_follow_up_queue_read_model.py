...
def _payload(run_id: str = "p17-shadow-follow-up-queue-2026-08-29") -> dict:
    return {
        "run_id": run_id,
        "follow_up_date": "2026-08-29",
        "status": "shadow_follow_up_queue_ready",
        "operator_id": "operator-a",
        "source_p16_decision_run_ids": ["p16-shadow-review-decisions-2026-08-29"],
        ...
        "items": [
            {
                "follow_up_item_id": "artifact-provided-id-must-be-ignored",
...
def test_load_shadow_follow_up_queue_read_model_rows_preserves_run_item_safety_and_lineage(tmp_path):
    ...
    rows = load_shadow_follow_up_queue_read_model_rows(json_path)

    assert rows["run"]["run_id"] == "p17-shadow-follow-up-queue-2026-08-29"
    assert rows["run"]["operator_id"] == "operator-a"
    assert rows["run"]["source_p16_decision_run_ids"] == ["p16-shadow-review-decisions-2026-08-29"]
    ...
    assert rows["run"]["items_csv_path"] == str(tmp_path / "operator_shadow_follow_up_queue_2026-08-29_items.csv")
    assert rows["run"]["markdown_path"] == str(tmp_path / "operator_shadow_follow_up_queue_2026-08-29.md")

    item = rows["items"][0]
    assert item["follow_up_item_id"].startswith(
        "operator_shadow_follow_up:p17-shadow-follow-up-queue-2026-08-29:"
    )
    assert item["follow_up_item_id"] != "artifact-provided-id-must-be-ignored"
    ...
    assert item["follow_up_status"] == "collect_more_evidence"
...
def test_load_shadow_follow_up_queue_read_model_rows_rejects_production_write_enabled(tmp_path):
    payload = _payload()
    ...
    with pytest.raises(ValueError, match="production_write_not_allowed"):
        load_shadow_follow_up_queue_read_model_rows(json_path)


def test_load_shadow_follow_up_queue_read_model_rows_rejects_item_production_write_enabled(tmp_path):
    payload = _payload()
...
def test_import_shadow_follow_up_queue_upserts_run_and_item(monkeypatch, tmp_path):
    from stock_research.operator_decision import shadow_follow_up_queue_read_model

    json_path = tmp_path / "operator_shadow_follow_up_queue_2026-08-29.json"
    json_path.write_text(json.dumps(_payload()), encoding="utf-8")
    conn = _Connection()
    monkeypatch.setattr(shadow_follow_up_queue_read_model, "connect", lambda service: _Context(conn))

    result = import_shadow_follow_up_queue(json_path, service="stock_research_test")

    assert result["imported_count"] == 1
    assert result["item_count"] == 1
    assert result["run_ids"] == ["p17-shadow-follow-up-queue-2026-08-29"]
    run_sql, run_params = conn.cursor_obj.calls[0]
    assert "INSERT INTO ops.operator_shadow_follow_up_run" in run_sql
    assert "ON CONFLICT (run_id)" in run_sql
    assert run_params["json_path"] == str(json_path)
    assert isinstance(run_params["source_p16_decision_run_ids"], str)
    item_sql, item_params = conn.cursor_obj.calls[1]
    assert "INSERT INTO ops.operator_shadow_follow_up_item" in item_sql
    assert "ON CONFLICT (follow_up_item_id)" in item_sql
    assert item_params["follow_up_status"] == "collect_more_evidence"


def test_import_shadow_follow_up_queue_directory_imports_sorted_artifacts(monkeypatch, tmp_path):
    from stock_research.operator_decision import shadow_follow_up_queue_read_model

    later_path = tmp_path / "operator_shadow_follow_up_queue_2026-09-30.json"
    earlier_path = tmp_path / "operator_shadow_follow_up_queue_2026-08-29.json"
    ignored_path = tmp_path / "not_a_p17_follow_up.json"
    later_path.write_text(json.dumps(_payload("p17-later")), encoding="utf-8")
    earlier_path.write_text(json.dumps(_payload("p17-earlier")), encoding="utf-8")
    ignored_path.write_text(json.dumps(_payload("ignored")), encoding="utf-8")
    conn = _Connection()
    monkeypatch.setattr(shadow_follow_up_queue_read_model, "connect", lambda service: _Context(conn))

    result = import_shadow_follow_up_queue(tmp_path, service="stock_research_test")

    assert result["imported_count"] == 2
    assert result["item_count"] == 2
    assert result["run_ids"] == ["p17-earlier", "p17-later"]
    run_params = [params for _sql, params in conn.cursor_obj.calls[::2]]
    assert [params["run_id"] for params in run_params] == ["p17-earlier", "p17-later"]
