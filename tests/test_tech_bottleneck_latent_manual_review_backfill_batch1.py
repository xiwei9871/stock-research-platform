...
SCRIPT = PROJECT_ROOT / "scripts/run_tech_bottleneck_latent_manual_review_backfill_batch1.py"
...
OUTPUT_DIR = PROJECT_ROOT / "outputs/research/tech_bottleneck_latent_manual_review_backfill_batch1_v1"
...
def test_latent_manual_review_backfill_batch1_outputs_and_guardrails() -> None:
    input_hashes_before = {
    ...
    expected = {
        "latent_manual_review_backfill_batch1_summary.json",
        "latent_manual_review_backfill_batch1_evidence.csv",
        "latent_manual_review_backfill_batch1_stock_status.csv",
        "latent_manual_review_backfill_batch1_page_citations.csv",
        "latent_manual_review_backfill_batch1_parse_gaps.csv",
        "latent_manual_review_backfill_batch1_guardrails.json",
        "tech_bottleneck_latent_manual_review_backfill_batch1_v1_report.md",
    }
    assert OUTPUT_DIR.exists()
    assert expected.issubset({path.name for path in OUTPUT_DIR.iterdir()})
    assert input_hashes_before == input_hashes_after

    summary = json.loads((OUTPUT_DIR / "latent_manual_review_backfill_batch1_summary.json").read_text(encoding="utf-8"))
    guardrails = json.loads((OUTPUT_DIR / "latent_manual_review_backfill_batch1_guardrails.json").read_text(encoding="utf-8"))

    assert summary["research_only"] is True
    assert summary["source_collection_batch1_stock_count"] == 26
    assert summary["source_collection_batch1_pdf_count"] == 78
    assert summary["processed_stock_count"] == 26
    assert summary["processed_pdf_count"] == 78
    ...
    assert summary["acceptance_decision"] in {
        "latent_manual_review_backfill_batch1_ready",
        "conditionally_ready_with_parse_gaps",
    }

    assert guardrails["research_only"] is True
    assert guardrails["source_collection_batch1_stock_count"] == 26
    assert guardrails["source_collection_batch1_pdf_count"] == 78
    assert guardrails["processed_stock_count"] == 26
    assert guardrails["processed_pdf_count"] == 78
...
def test_latent_manual_review_backfill_batch1_scope_and_outputs() -> None:
    _run_generator()
    ...
    evidence = pd.read_csv(OUTPUT_DIR / "latent_manual_review_backfill_batch1_evidence.csv", dtype={"stock_code": str})
    stock_status = pd.read_csv(OUTPUT_DIR / "latent_manual_review_backfill_batch1_stock_status.csv", dtype={"stock_code": str})
    citations = pd.read_csv(OUTPUT_DIR / "latent_manual_review_backfill_batch1_page_citations.csv", dtype={"stock_code": str})
    parse_gaps = pd.read_csv(OUTPUT_DIR / "latent_manual_review_backfill_batch1_parse_gaps.csv", dtype={"stock_code": str})

    expected_codes = set(queue["stock_code"].astype(str).str.zfill(6))
    assert len(queue) == 26
    assert len(stock_status) == 26
...
def test_latent_manual_review_backfill_batch1_deterministic_and_strategy_diff_clean() -> None:
    _run_generator()
