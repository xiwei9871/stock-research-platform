...
SCRIPT = PROJECT_ROOT / "scripts/run_tech_bottleneck_likely_core_36_primary_source_backfill.py"
...
OUTPUT_DIR = PROJECT_ROOT / "outputs/research/tech_bottleneck_likely_core_36_primary_source_backfill_v1"
...
def test_likely_core_36_primary_source_backfill_outputs_and_guardrails() -> None:
    queue_hash_before = _sha(INPUT_QUEUE)
    _run_generator()
    queue_hash_after = _sha(INPUT_QUEUE)

    expected = {
        "likely_core_36_primary_source_backfill_summary.json",
        "likely_core_36_backfill_results.csv",
        "likely_core_36_primary_source_evidence_matrix.csv",
        "likely_core_36_gap_matrix.csv",
        "likely_core_36_upgrade_candidates.csv",
        "likely_core_36_remain_pending_candidates.csv",
        "likely_core_36_adjacent_or_downgrade_candidates.csv",
        "likely_core_36_primary_source_backfill_guardrails.json",
        "tech_bottleneck_likely_core_36_primary_source_backfill_v1_report.md",
    }
    assert OUTPUT_DIR.exists()
    assert expected.issubset({path.name for path in OUTPUT_DIR.iterdir()})
    assert queue_hash_before == queue_hash_after

    summary = json.loads((OUTPUT_DIR / "likely_core_36_primary_source_backfill_summary.json").read_text(encoding="utf-8"))
    guardrails = json.loads((OUTPUT_DIR / "likely_core_36_primary_source_backfill_guardrails.json").read_text(encoding="utf-8"))

    assert summary["source_likely_core_queue_count"] == 36
    assert summary["processed_count"] == 36
    assert summary["upgrade_count"] + summary["remain_pending_count"] + summary["adjacent_count"] + summary["downgrade_or_reject_count"] == 36
    ...
    assert summary["acceptance_decision"] in {
        "likely_core_36_primary_source_backfill_ready",
        "conditionally_ready_with_remaining_evidence_gaps",
    }

    assert guardrails["research_only"] is True
    assert guardrails["source_likely_core_queue_count"] == 36
    assert guardrails["only_likely_core_queue_processed"] is True
...
def test_likely_core_36_primary_source_backfill_processes_only_36_queue_rows() -> None:
    _run_generator()

    queue = pd.read_csv(INPUT_QUEUE, dtype={"stock_code": str})
    results = pd.read_csv(OUTPUT_DIR / "likely_core_36_backfill_results.csv", dtype={"stock_code": str})
    evidence = pd.read_csv(OUTPUT_DIR / "likely_core_36_primary_source_evidence_matrix.csv", dtype={"stock_code": str})
    gaps = pd.read_csv(OUTPUT_DIR / "likely_core_36_gap_matrix.csv", dtype={"stock_code": str})
    ...
    assert results["previous_manual_review_entry_class"].eq("likely_core_pending_evidence").all()
    ...
    allowed_decisions = {
        "upgrade_to_confirmed_core_manual_approval_candidate",
...
def test_likely_core_36_primary_source_backfill_deterministic_and_strategy_diff_clean() -> None:
    _run_generator()
