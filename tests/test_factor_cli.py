...
def test_p16_import_shadow_review_decisions_parser_accepts_path():
    args = cli.build_parser().parse_args(
        ["p16-import-shadow-review-decisions", "--path", "outputs/p16"]
    )

    assert args.command == "p16-import-shadow-review-decisions"
    assert args.path == "outputs/p16"
    assert args.service == "stock_research"


def test_p16_import_shadow_review_decisions_dispatches(monkeypatch, capsys):
    captured = {}

    def fake_import_shadow_review_decisions(path, service):
        captured["path"] = path
        captured["service"] = service
        return {"imported_count": 2, "group_count": 3, "run_ids": ["p16-a", "p16-b"]}

    monkeypatch.setattr(cli, "import_shadow_review_decisions", fake_import_shadow_review_decisions)
    ...
    output = capsys.readouterr().out
    assert "p16_import_shadow_review_decisions|imported|2" in output
    assert "p16_import_shadow_review_decisions|groups|3" in output
    assert "p16_import_shadow_review_decisions|runs|p16-a,p16-b" in output
    assert captured == {"path": "outputs/p16", "service": "stock_research_test"}


def test_p17_import_shadow_follow_up_queue_parser_accepts_path():
    args = cli.build_parser().parse_args(
        ["p17-import-shadow-follow-up-queue", "--path", "outputs/p17"]
    )

    assert args.command == "p17-import-shadow-follow-up-queue"
    assert args.path == "outputs/p17"
    assert args.service == "stock_research"


def test_p17_import_shadow_follow_up_queue_dispatches(monkeypatch, capsys):
    captured = {}

    def fake_import_shadow_follow_up_queue(path, service):
        captured["path"] = path
        captured["service"] = service
        return {"imported_count": 2, "item_count": 3, "run_ids": ["p17-a", "p17-b"]}

    monkeypatch.setattr(cli, "import_shadow_follow_up_queue", fake_import_shadow_follow_up_queue)
    ...
    output = capsys.readouterr().out
    assert "p17_import_shadow_follow_up_queue|imported|2" in output
...
def test_strategy_score_audit_cli_failed_summary_does_not_print_fake_detail(monkeypatch, capsys, tmp_path):
    monkeypatch.setattr(
        cli,
        "load_strategy_score_audit_summary",
        lambda **kwargs: {
