...
def test_build_daily_pipeline_steps_lists_required_initial_steps() -> None:
    ...
    assert steps[-2].required is False
...
def test_run_stock_daily_data_pipeline_keeps_intraday_feature_failure_nonblocking(
    tmp_path: Path,
) -> None:
    def fake_runner(command: list[str], timeout_seconds: int) -> dict[str, object]:
        if "build-intraday-features-daily" in command:
            return {"returncode": 1, "stdout": "", "stderr": "intraday failed"}
        return {"returncode": 0, "stdout": "rows|10", "stderr": ""}
    ...
    assert result["status"] == "success"
    steps = {step["step"]: step for step in result["steps"]}
    assert steps["intraday_feature_build"]["status"] == "failed"
    assert steps["daily_event_refresh"]["status"] == "success"
