...
def test_build_sector_heatmap_payload_returns_concept_items(monkeypatch):
    monkeypatch.setattr(
        sector_heatmap_service,
        "load_sector_heatmap_rows",
        lambda trade_date, sector_type, service=None: [
            {
                ...
                "close": 112.0,
                "preclose": 100.0,
                "amount": 128000000000.0,
                "up_count": 68,
                "down_count": 12,
                "stock_count": 91,
                "source": "market.concept_daily_bar",
                "updated_at": "2026-06-26T15:45:00+08:00",
            }
        ],
    )
    ...
    assert payload["data_status"] == "completed"
    assert payload["warnings"] == []
    assert payload["source"] == "market.concept_daily_bar"
    assert payload["items"][0]["sector_id"] == "THS_AI"
    assert payload["items"][0]["sector_name"] == "人工智能"
    assert payload["items"][0]["sector_type"] == "concept"
    assert payload["items"][0]["change_pct"] == pytest.approx(0.12)
    assert payload["items"][0]["amount"] == 128000000000.0
...
def test_load_sector_fund_flow_rows_queries_derived_concept_proxy(monkeypatch):
    calls: list[dict[str, object]] = []
        ...
        return [
            {
                ...
                "preclose": 100.0,
                "amount": 128000000000.0,
                "main_net_inflow": 15360000000.0,
                "main_net_inflow_ratio": 0.12,
                "leading_stock_name": "寒武纪",
                "source": "derived:concept_amount_price_breadth_proxy",
                "updated_at": "2026-06-26T15:45:00+08:00",
            }
        ]
    ...
    assert rows[0]["concept_code"] == "THS_AI"
    assert calls[0] == {"service": "stock_research"}
    assert calls[1]["params"] == [
        "2026-06-26",
        "2026-06-26",
    ...
    assert "derived:concept_amount_price_breadth_proxy" in str(calls[1]["sql"])
    assert "market.concept_daily_bar" in str(calls[1]["sql"])
...
def test_build_sector_detail_payload_returns_concept_detail(monkeypatch):
    monkeypatch.setattr(
        sector_detail_service,
        "load_sector_detail_row",
        lambda trade_date, sector_id, sector_type, service=None: {
            "trade_date": trade_date,
            "concept_code": sector_id,
            "concept_name": "人工智能",
            "close": 112.0,
            "preclose": 100.0,
            "amount": 128000000000.0,
        ...
        lambda trade_date, sector_id, sector_type, service=None, limit=10: [
            {
                ...
                "pct_chg": 12.5,
                "source": "market_daily_bar",
                "updated_at": "2026-06-26T15:46:00+08:00",
            }
        ],
    )
    ...
    assert payload["data_status"] == "completed"
    assert payload["warnings"] == []
    assert payload["source"] == "market.concept_daily_bar,market_daily_bar"
    assert payload["sector_id"] == "THS_AI"
    assert payload["sector_name"] == "人工智能"
    assert payload["sector_type"] == "concept"
    assert payload["change_pct"] == pytest.approx(0.12)
    assert payload["leading_stocks"][0]["asset_id"] == "688256.SH"
