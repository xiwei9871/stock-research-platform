...
SCRIPT = PROJECT_ROOT / "scripts/run_tech_bottleneck_latent_manual_review_equivalence_gate_batch1.py"
...
OUTPUT_DIR = PROJECT_ROOT / "outputs/research/tech_bottleneck_latent_manual_review_equivalence_gate_batch1_v1"
...
def test_latent_manual_review_equivalence_gate_batch1_outputs_and_guardrails() -> None:
    input_hashes_before = {
    ...
    expected = {
        "latent_manual_review_equivalence_gate_batch1_summary.json",
        "latent_manual_review_equivalence_gate_batch1_decisions.csv",
        "latent_manual_review_equivalence_gate_batch1_core_equivalent_proposals.csv",
        "latent_manual_review_equivalence_gate_batch1_keep_separate.csv",
        "latent_manual_review_equivalence_gate_batch1_human_confirm_required.csv",
        "latent_manual_review_equivalence_gate_batch1_downgrade_or_reject.csv",
        "latent_manual_review_equivalence_gate_batch1_guardrails.json",
        "tech_bottleneck_latent_manual_review_equivalence_gate_batch1_v1_report.md",
    }
    ...
    summary = json.loads(
        (OUTPUT_DIR / "latent_manual_review_equivalence_gate_batch1_summary.json").read_text(encoding="utf-8")
    )
    guardrails = json.loads(
        (OUTPUT_DIR / "latent_manual_review_equivalence_gate_batch1_guardrails.json").read_text(encoding="utf-8")
    )

    assert summary["research_only"] is True
    assert summary["source_backfill_batch1_stock_count"] == 26
    assert summary["processed_stock_count"] == 26
    assert summary["source_primary_source_supported_count"] == 26
    assert (
    ...
    assert summary["quality_pool_v6_generated"] is False
    ...
    assert summary["acceptance_decision"] in {
        "latent_manual_review_equivalence_gate_batch1_ready",
        "conditionally_ready_with_human_confirm_needed",
    }

    assert guardrails["research_only"] is True
    assert guardrails["source_backfill_batch1_stock_count"] == 26
    assert guardrails["processed_stock_count"] == 26
    assert guardrails["source_primary_source_supported_count"] == 26
    ...
    assert guardrails["quality_pool_v6_generated"] is False
...
def test_latent_manual_review_equivalence_gate_batch1_scope_and_integrity() -> None:
    _run_generator()
    ...
    decisions = pd.read_csv(
        OUTPUT_DIR / "latent_manual_review_equivalence_gate_batch1_decisions.csv",
        dtype={"stock_code": str},
    )
    core = pd.read_csv(
        OUTPUT_DIR / "latent_manual_review_equivalence_gate_batch1_core_equivalent_proposals.csv",
        dtype={"stock_code": str},
    )
    keep = pd.read_csv(
        OUTPUT_DIR / "latent_manual_review_equivalence_gate_batch1_keep_separate.csv",
        dtype={"stock_code": str},
    )
    confirm = pd.read_csv(
        OUTPUT_DIR / "latent_manual_review_equivalence_gate_batch1_human_confirm_required.csv",
        dtype={"stock_code": str},
    )
    reject = pd.read_csv(
        OUTPUT_DIR / "latent_manual_review_equivalence_gate_batch1_downgrade_or_reject.csv",
        dtype={"stock_code": str},
    )
    ...
    assert len(core) + len(keep) + len(confirm) + len(reject) == 26
    ...
    assert decisions["quality_pool_v6_generated"].eq(False).all()
...
def test_latent_manual_review_equivalence_gate_batch1_deterministic_and_strategy_diff_clean() -> None:
    _run_generator()
