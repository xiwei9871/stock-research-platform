...
SCRIPT = PROJECT_ROOT / "scripts/run_tech_bottleneck_latent_core_equivalence_gate_batch1.py"
INPUT_CANDIDATES = (
    PROJECT_ROOT
...
OUTPUT_DIR = PROJECT_ROOT / "outputs/research/tech_bottleneck_latent_core_equivalence_gate_batch1_v1"
...
def test_latent_core_equivalence_gate_batch1_outputs_and_guardrails() -> None:
    input_hash_before = _sha(INPUT_CANDIDATES)
    _run_generator()
    input_hash_after = _sha(INPUT_CANDIDATES)

    expected = {
        "latent_core_equivalence_gate_batch1.csv",
        "latent_core_equivalence_batch1_summary.json",
        "latent_core_equivalent_batch1_candidates.csv",
        "latent_keep_separate_batch1_candidates.csv",
        "latent_remain_watch_batch1.csv",
        "latent_downgrade_or_reject_batch1.csv",
        "latent_core_equivalence_batch1_guardrails.json",
        "tech_bottleneck_latent_core_equivalence_gate_batch1_v1_report.md",
    }
    assert OUTPUT_DIR.exists()
    assert expected.issubset({path.name for path in OUTPUT_DIR.iterdir()})
    assert input_hash_before == input_hash_after

    summary = json.loads((OUTPUT_DIR / "latent_core_equivalence_batch1_summary.json").read_text(encoding="utf-8"))
    guardrails = json.loads((OUTPUT_DIR / "latent_core_equivalence_batch1_guardrails.json").read_text(encoding="utf-8"))

    assert summary["source_latent_manual_approval_candidate_count"] == 4
    ...
    assert summary["acceptance_decision"] in {
        "latent_core_equivalence_gate_batch1_ready",
        "conditionally_ready_with_equivalence_gaps",
    }

    assert guardrails["research_only"] is True
    assert guardrails["source_latent_manual_approval_candidate_count"] == 4
    assert guardrails["only_latent_manual_approval_candidates_processed"] is True
    assert guardrails["latent_pending_45_processed"] is False
...
def test_latent_core_equivalence_gate_batch1_integrity() -> None:
    _run_generator()

    candidates = pd.read_csv(INPUT_CANDIDATES, dtype={"stock_code": str})
    gate = pd.read_csv(OUTPUT_DIR / "latent_core_equivalence_gate_batch1.csv", dtype={"stock_code": str})
    core = pd.read_csv(OUTPUT_DIR / "latent_core_equivalent_batch1_candidates.csv", dtype={"stock_code": str})
    keep = pd.read_csv(OUTPUT_DIR / "latent_keep_separate_batch1_candidates.csv", dtype={"stock_code": str})
    watch = pd.read_csv(OUTPUT_DIR / "latent_remain_watch_batch1.csv", dtype={"stock_code": str})
    reject = pd.read_csv(OUTPUT_DIR / "latent_downgrade_or_reject_batch1.csv", dtype={"stock_code": str})
    ...
    assert len(core) + len(keep) + len(watch) + len(reject) == 4


def test_latent_core_equivalence_gate_batch1_deterministic_and_strategy_diff_clean() -> None:
    _run_generator()
