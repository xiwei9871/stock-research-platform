...
describe('strategy chart markers', () => {
  ...
  it('labels the first intraday bar of each trade date with the date', () => {
    ...
    expect(formatter(data[0].time as Time, 0, 'zh-CN')).toBe('06-03');
    expect(formatter(data[1].time as Time, 0, 'zh-CN')).toBe('10:30');
    expect(formatter(data[2].time as Time, 0, 'zh-CN')).toBe('06-04');
  });

  it('uses dense synthetic times for intraday bars while formatting original exchange time', () => {
    render(
      createElement(AssetChart, {
        bars: [
          bar({ time: '2026-06-18 15:00:00' }),
          bar({ time: '2026-06-29 09:35:00' })
        ],
        timeAxisMode: 'intraday'
      })
    );
    ...
    const secondTime = data[1].time as number;

    expect(secondTime - firstTime).toBe(60);
    ...
    >;
    expect(calls[0][1].localization.timeFormatter(secondTime as Time)).toBe('2026-06-29 09:35');
  });
});
