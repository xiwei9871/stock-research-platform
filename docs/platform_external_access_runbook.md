...
`THEME_RESEARCH_REPORT_HOST_ROOT` is a mandatory, safe absolute path on the remote host. It must already exist as a readable/traversable directory; the release script never creates it and never writes report artifacts. Compose mounts it as `${THEME_RESEARCH_REPORT_HOST_ROOT}:/app/reports/theme-research:ro` and fixes `THEME_RESEARCH_REPORT_ROOT=/app/reports/theme-research`, so every rebuild and rollback keeps the same container path and persistent host tree. Before `compose build` the sync fails closed if the host directory is missing, not a directory, or unreadable. After startup the container check requires the path to be readable and the filesystem to report `ST_RDONLY`; it does not probe read-only status by attempting a write.
