...
DASHBOARD_LOGIN_PASSWORD=application-password
...
`REMOTE_USER`, `REMOTE_HOST`, and `REMOTE_DIR` retain the existing defaults (`jqz`, `192.168.3.185`, and `/home/$REMOTE_USER/code/stock-research-platform-main`) but should be set explicitly outside that host. SSH defaults to `BatchMode=yes`; use a dedicated host entry through `STOCK_RESEARCH_SSH_CONFIG`. Legacy `SSH_OPTS` remains an explicit compatibility override, accepts only simple option tokens, and should not be used to restore password-only automation. The script verifies SSH and `docker compose` before the first `rsync`. `DASHBOARD_AUTH` is the outer HTTP Basic credential, while `DASHBOARD_LOGIN_USERNAME` and `DASHBOARD_LOGIN_PASSWORD` authenticate the application session used by protected release probes. These values are passed only to the release check and must not be committed or embedded in the frontend. The gate escapes Basic auth into a `0600 temporary curl configuration`, writes the application login JSON and cookie jar under the same private temporary directory, and removes the directory with an exit trap; only temporary file paths, never credentials, appear in curl process arguments.

The release sync uses only the complete version-controlled `deploy/dashboard-release.compose.yml`; it never merges an unknown remote Compose file. Every Compose command uses the explicit `STOCK_RESEARCH_COMPOSE_PROJECT` name (default `stock_research_dashboard`). The canonical stack builds both images, binds the API to host `127.0.0.1:8765`, binds the frontend to host `127.0.0.1:5174`, and connects them on the dedicated `stock-research-dashboard-release` network. The frontend Nginx container proxies `/api/` to `api:8765`. The server's canonical `artifacts/theme_decomposition` tree is mounted read-only at `/app/artifacts/theme_decomposition`; canonical theme artifacts remain outside the image, while the release gate requires at least 25 unique themes and the approved AI-compute theme. Server-only application variables come from `DASHBOARD_REMOTE_ENV_FILE`; PostgreSQL service configuration comes from `DASHBOARD_PGSERVICE_FILE`. Both files must already exist on the remote and are never synchronized from the repository. `PGSERVICEFILE` must provide distinct runtime, indexer, and reviewer aliases. Each underlying login must receive only the matching `theme_research_runtime`, `theme_research_report_indexer`, or `theme_research_report_reviewer` NOLOGIN role membership (or set that role with a libpq `options=-c role=...` entry). Production should use independent LOGIN credentials for the three aliases so credential compromise cannot merge capabilities. The canary reports both `current_user` and `session_user`, rejects reused LOGIN identities, and recursively reads `pg_auth_members` to enforce no additional role membership—direct, indirect, unknown, or cyclic—for each LOGIN and no inherited roles for the capability roles themselves. It records `server_version_num`: PostgreSQL 16 and later use `pg_has_role(..., 'SET')`, while PostgreSQL 15 compatibility uses the supported `MEMBER` semantics and never sends the unsupported `SET` token.
