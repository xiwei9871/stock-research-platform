...
- With a nonempty boundary, every formal read API that defaults its trade date—Review Queue, Market Monitor EOD, Daily Review Lite, Evidence Digest, and Generated Reports within Global Search—uses the same manifest display-date resolver. Missing/error/no-ready manifests must return the prior ready date or a safe unavailable/empty result and must never read or create candidate-date artifacts; explicit historical dates are unchanged.

Do not enable either value until the controlled rollout checklist in [Playwright Platform Validation](playwright-platform-validation.md) is complete. In particular, run and verify `apply_data_run_manifest_schema()` first so the degraded browser-acceptance CHECK migration is installed; an unverified or failed migration keeps rollout `BLOCKED`. Then enable `STOCK_RESEARCH_EOD_BROWSER_ACCEPTANCE_ENABLED=true` and the matching `STOCK_RESEARCH_BROWSER_ACCEPTANCE_REQUIRED_FROM` boundary in the same rollout change. When browser acceptance is enabled, `DASHBOARD_CACHE_CLEAR_URL` must target the exact local-only path `/api/dashboard/cache/clear` on a literal IPv4/IPv6 loopback address. If login credentials are configured, `DASHBOARD_AUTH_LOGIN_URL` must use the same scheme, literal address, and effective port with the exact path `/api/auth/login`. Userinfo, query strings, fragments, DNS names (including `localhost`), cross-origin login, redirects, invalid ports, and environment proxies are rejected. Any unset or unsafe URL makes a repairable cache-clear attempt fail as infrastructure rather than silently skipping it.
