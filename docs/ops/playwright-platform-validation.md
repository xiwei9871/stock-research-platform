...
- Before tests, run a scanner self-test through the full `command_secret_scan` entry point using a real-shape `trace.zip` with an `abc-trace.network` JSONL member. Require cookie, authorization, X-CSRF-token, and password fixtures to classify as potential real credentials, the synthetic fixture to remain synthetic, and a malformed line to become an expected line-specific scan error. Scan `inputs/raw`, copied artifact/report trees, `inputs/report-ready`, the generated report/evidence, recursive JSON keys and header name/value pairs, and text members inside ZIP traces. Treat filesystem and ZIP-member `.jsonl`, `.network`, and `.trace` files as JSONL: parse every nonempty line and never silently fall back when parsing fails. Record all size-limit skips and reasons. Classify findings as known synthetic sentinels, known noncredential framework code, potential real credentials, or scan errors. Framework classification may use only a fixed token-hash allowlist whose Playwright version exactly matches the source frozen inputs; path-level framework whitelists are prohibited. Potential real credentials or scan errors block sealing; expected synthetic sentinels keep the candidate untrusted and support the R5 ledger.
...
Source seal hashes are core `b01c1ead7d135db8c54a73afdbd4aa7215119d2b50670d895d7fdcdaf2f73409`, artifact manifest `0f0fa33fa99c934b291bdcffd480bea95892b786c2b50d20f52f6d61beb7688b`, and tree `1916ccdcf3d384cc8c2f1073e92668a11058c7811852a33b827b341bceb97473`. Security-rescan hashes are core `7bef47b74fffc0a79e648b0abd8c5e6e32f33e1ac585308fb8bba28b529017e3`, artifact manifest `6af2740c6dbd856b01ba7a0594e309e30ce543906c1f26c4b241cb7aedce9110`, and tree `1c3a773fdd8db65bd06e2e56b426118ecf3ee49498cbb25012a18b3daf56faf1`.

The earlier `pv-initial-20260720-372f4a5`, `pv-initial-20260721-5fb90fd`, and `pv-initial-20260721-796495a` directories are retained unchanged as superseded attempts. The fourth run is retained unchanged as the amended execution source; the fifth directory supplies the authoritative security seal.
